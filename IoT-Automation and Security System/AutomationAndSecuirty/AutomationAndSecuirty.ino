/*
   ...
   Make sure to install all libraries before proceeding
   ...
   Upload RTC Set time code before this to set RTC Time or whenever the time is wrong.
   Update the code with your unique RFID Card ID using RFID Code.
...
const char mqttPass[] = "";
const char clientID[] = "9899845428"; // Use your number or anything unique. Must be unique in the world.
...
int servoPin = 13;
int pirPin = 26;
const int R = 3, G = 4, B = 5; //PWM chanles
...
void wifiServer() {
  ...
  if (s != "") {
    ...
    if (s.indexOf("IN") >= 0) {
      ...
      Serial.print("Servo angle set to ");
      Serial.println(i);
      myservo.write(i); // Set initial angle to zero.
    }
...
// Servo function
void servoSetup() {
  ESP32PWM::allocateTimer(0); // Dont use pwm channel 0,1,8,9 elsewhere
  myservo.setPeriodHertz(50);// Standard 50hz servo
  Serial.print("Servo attached to channel ");
  Serial.println(myservo.attach(servoPin));
