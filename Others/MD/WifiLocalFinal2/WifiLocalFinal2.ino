...
void loop()
{
  String s = appHandler(); // receving the data
  if (s != "") {
    // Manipulate the string according to your needs
    Serial.println(s);
    String pin, command;
    if (s.indexOf("IN") >= 0) { // s=PIN2ON
      pin = s.substring(3, 4);
    ...
    if (s.indexOf("deepak") >= 0) {
      Serial.print("Brightness is set to ");
      Serial.println(s.substring(6, 8));
    }
    delay(10);
  }
  motionSensor();
}
void appData(WiFiClient cl) { // Sending the data
  //To read
  cl.println("<br/><a id=" + id1 + ">" + String(mesg1) + "</a>");
  cl.println("<br/><a id=" + id2 + ">" + String(mesg2) + "</a>");
  cl.println("<br/><a id=" + id3 + ">" + String(motionFlag) + "</a>");
}
